var DesignHelper = {
    ...
    init: function () {
        var $this = this;
        $this._map();
        $this.map.toggleSideMenu.click(function () {
            return $this.resizeSideMenu();
        });
        $this.map.window.resize(function () {
            return $this.windowResize();
        });
        $this.map.dropDownSideBar.on('click', function () {
            return $this.dropDownShow(this);
        });
        $this.map.dropDownLiSideBar.on("show.bs.dropdown", function () {
            $this.sidebarWrapperSize(250);
        });
        $this.map.dropDownLiSideBar.on("hide.bs.dropdown", function () {
            $this.sidebarWrapperSize('');
        });
        $this.map.leftSideBar.scroll(function () {
            return $this.scrollBarPosition(this);
        });
    },
    ...
    scrollBarPosition: function (link) {
        var sidebarHeight = $('.sidebar-left-content.nano-content').height(),
            menuHeight = $('#sidebar_left-collapse').height(),
            div = (menuHeight + 40) / sidebarHeight;

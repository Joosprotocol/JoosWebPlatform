var CatalogHelper = {
    ...
    init: function () {
        ...
        $this.map.filterCategory.on('change', function (element) {
            $this.changeFilterCategory(element);
        });
        $this.map.dynamicFormWrapper.on("afterInsert", function (e, item) {
            $this.checkItemNumber();
        });
        $this.map.dynamicFormWrapper.on("afterDelete", function (e) {
            $this.checkItemNumber();
        });
        $this.map.dynamicFormWrapper.on("beforeDelete", function (e) {
            return $this.beforeDeleteItem();
        });
    },
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                for (item in response.list) {
                    $this.map.filterField.append($('<option>', {
                        value: item,
                        selected: item == field,
                        text: response.list[item]
                    }));
                }
            }
        });
